...
*Замечание.* Если матрица $A$ допускает ц р, то можно, переставляя её строки и столбцы, привести её к блочному виду
...
<ol>
<li> \(A\) устойчивая матрица </li>

<li> \(A\) не допускает циклических разложений </li>

<li> \(\text{НОК}\{t | [A^t]_{ii} > 0\} = 1\) \(\Rightarrow\) это будет так же для любого \(i\) </li>
...
0 & 1 \\
1 & 0
\end{pmatrix}\) не является устойчивой, она не допускает циклических разложений, ... остальные свойства также верны 
...
$$\sum_{i=1}^n = 1, \quad j=1,\dots,n \tag{✻✻}$$
...
*Упражнение.* Доказать, что из (✻✻) следует $$\lambda(A) = 1$$
